...
get_total_incidence = function(output, parameters) {
  with(as.list( parameters), {
    incid =  output %>% mutate(inc_res = sigma*(Er+Erv+Erw), 
                               inc_mut = sigma*(Em +Emv +Emw), 
                               inc_tot = sigma*(Er+Erv+Erw+Em +Emv +Emw), 
...
make_init = function( N=5.07e6, vaxlevel = 0.8,
                      port_wane = 0.1, 
                      past_infection = 0.1, incres = 500, incmut = 20, 
                      pars=as.list(parameters)) {
  ff=2/3 # fudge factor . hard to get incidence right since it depends on other pars too 
...
parameters <-         c(sigma=1/3, # incubation period (3 days) (to fixed)
                        ...
                        beta_r=0.35, #transmission rate (to estimate)
                        beta_m=0.35*1.9, #transmission rate (to estimate)
                        epsilon_r = (1-0.8), # % this should be 1-ve 
                        epsilon_m = (1-0.4), # % escape capacity #(fixed)
