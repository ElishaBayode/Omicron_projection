...
guess <- c(log(0.5), logit(0.52), log(2.1), log(0.15))

#the parameters are constrained  accordingly (lower and upper)

fit_MB <- optim(fn=f_loglik,par=guess, lower=c(log(0.4), 0.1, log(1.7), log(0.1)), 
                upper = c(log(0.7), 0.6, log(2), log(0.3)), method = "L-BFGS-B")
...
#an attempt to model changes in ascertainment probMBility 
...
fit_MB2 <- optim(fn=f_loglik,par=guess, lower=c(log(0.2), log(0.001)), 
                 upper = c(log(0.6), log(0.7)), method = "L-BFGS-B")


#this catches estimated parameter values from MLE 
mle_est_MB2 <- c(beta_m=exp(fit_MB2$par[1]),p=expit(fit_MB$par[2]), beta_r=exp(fit_MB2$par[3]),theta=exp(fit_MB$par[4]))

signif(mle_est_MB2,3)
...
coef(pomp_obj) <- c(c(S_0=last(out_state["S",,]),Er_0=last(out_state["Er",,]),Em_0=last(out_state["Em",,]),
                      ...
                      c(parameters,mle_est_MB2)))


#


estim_parameters <- c(parameters_1,mle_est_MB2)
...
estim_parameters <- c(parameters_2,mle_est_MB2)

MB_forecast_int <- case_projection_rel(out_state=out_state,
                                       forecast_days=30, 
                                       lag=0,
                                       parameters_estim = estim_parameters, 
                                       simu_size = 100000,
...
saveRDS(dat_sim_rel, file.path("data/MB_dat_sim_rel.rds"))
dat_sim_rel =readRDS(file.path("data/MB_dat_sim_rel.rds"))
...
gg_MB <- ggplot() + geom_line(data=dat_sim, aes(x=date ,y =`50%`),color='blue',size=1.2,alpha=0.4) +
  ...
  geom_line(data=MB_forecast,aes(x=date, y=`50%`,color="Current, with testing constraints"),size=1.2,alpha=0.5) +
  geom_ribbon(data=MB_forecast,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill='darkgreen',alpha=0.1)+
  geom_line(data=MB_forecast_int,aes(x=date, y=`50%`, color="Without testing constraints"),size=1.2,alpha=0.5) +
  geom_ribbon(data=MB_forecast_int,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill='orange',alpha=0.1)+
  #geom_line(aes(y=typical),color='blue') +
  labs(y="Reported cases",x="Date") + ylim(c(0,11500)) + 
