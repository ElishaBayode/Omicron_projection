...
test_prop_QC <- filter(mytest_QC, date >= intro_date)$test_prop
test_prop = test_prop_QC
...
guess <- c(log(0.8), logit(0.52), log(2.9), log(0.11))
...
mle_est_QC <- c(beta_r=exp(fit_QC$par[1]),p=expit(fit_QC$par[2]), beta_m=exp(fit_QC$par[3]),theta=exp(fit_QC$par[4]))
...
coef(pomp_obj ) <- c(c(S_0=init[[1]],Er_0=init[[2]],Em_0=init[[3]],Ir_0=init[[4]],
                       ...
                       Imw_0=init[[17]],Rw_0=init[[18]] ,c(parameters,mle_est_QC)))
...
estim_parameters <- c(parameters_1,mle_est_QC)


#an attempt to model changes in ascertainment probQCility 
...
#this catches estimated parameter values from MLE 
mle_est_QC2 <- c(beta_m=exp(fit_QC2$par[1]),p=expit(fit_QC$par[2]), beta_r=exp(fit_QC2$par[3]),theta=exp(fit_QC$par[4]))

signif(mle_est_QC2,3)
...
coef(pomp_obj) <- c(c(S_0=last(out_state["S",,]),Er_0=last(out_state["Er",,]),Em_0=last(out_state["Em",,]),
                      ...
                      c(parameters,mle_est_QC2)))


#


estim_parameters <- c(parameters_1,mle_est_QC2)
...
######## forecast cases ############### 
QC_forecast <- case_projection(out_state=out_state_2,
                               forecast_days=30, 
                               lag=0,
                               test_prop = test_prop_QC,
...
estim_parameters <- c(parameters_2,mle_est_QC2)

QC_forecast_int <- case_projection_rel(out_state=out_state,
                                       forecast_days=30, 
                                       lag=0,
                                       parameters_estim = estim_parameters, 
                                       simu_size = 100000,
...
saveRDS(dat_sim, file.path("data/QC_dat_sim.rds"))
dat_sim =readRDS(file.path("data/QC_dat_sim.rds"))

saveRDS(dat_sim_rel, file.path("data/QC_dat_sim_rel.rds"))
dat_sim_rel =readRDS(file.path("data/QC_dat_sim_rel.rds"))

saveRDS(QC_forecast, file.path("data/QC_forecast.rds"))
QC_forecast =readRDS(file.path("data/QC_forecast.rds"))

saveRDS(QC_forecast_int, file.path("data/QC_forecast_int.rds"))
QC_forecast_int =readRDS(file.path("data/QC_forecast_int.rds"))


cols <- c("Current, with testing constraints" = "darkgreen", "Without testing constraints"="orange")

gg_QC <- ggplot() + geom_line(data=dat_sim, aes(x=date ,y =`50%`),color='blue',size=1.2,alpha=0.4) +
  ...
  geom_line(data=QC_forecast,aes(x=date, y=`50%`,color="Current, with testing constraints"),size=1.2,alpha=0.5) +
  geom_ribbon(data=QC_forecast,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill='darkgreen',alpha=0.1)+
