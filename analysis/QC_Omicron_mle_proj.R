...
guess <- c(log(0.8), logit(0.52), log(2.7), log(0.11))

#the parameters are constrained  accordingly (lower and upper)

fit_QC <- optim(fn=f_loglik,par=guess, lower=c(log(0.3), 0.1, log(1.9), log(0.1)), 
                upper = c(log(0.0), 0.6, log(2.9), log(0.12)), method = "L-BFGS-B")
...
test_prop <- 1 - 0.4/(1+ exp(-1.25*(1:nrow(dat_omic)-20)))
...
######## forecast cases ############### 
QC_forecast <- case_projection(out_state=out_state,
                               forecast_days=30, 
                               lag=0,
...
estim_parameters <- c(parameters_2,mle_est_QC)

QC_forecast_int <- case_projection(out_state=out_state,
                                   forecast_days=30, 
                                   lag=0,
                                   parameters_estim = estim_parameters, 
                                   simu_size = 100000,
...
gg_QC <- ggplot() + geom_line(data=dat_sim,aes(x=date,y=`50%`),color='blue',size=1.2,alpha=0.4) +
  ...
  geom_line(data=QC_forecast,aes(x=date, y=`50%`,color="Current"),size=1.2,alpha=0.5) +
  geom_ribbon(data=QC_forecast,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill='orange',alpha=0.1)+
  geom_line(data=QC_forecast_int,aes(x=date, y=`50%`, color="50% reduction in transmission"),size=1.2,alpha=0.5) +
  geom_ribbon(data=QC_forecast_int,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill='green',alpha=0.1)+
