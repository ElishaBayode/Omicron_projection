...
make_init = function( N=1.36e6, vaxlevel = 0.78,
                      port_wane = 0.1, 
                      past_infection = 0.1, incres = 250, incmut = 5, 
...
parameters <-         c(sigma=1/3, # incubation period (3 days) (to fixed)
                       ...
                       # beta_r=0.52, #transmission rate (to estimate) (0.35)
                        beta_m=0.76*2.2, #transmission rate (to estimate)(*1.9)
...
output_MB <- data.frame(time=ag$time ,S=ag$S[,1],Er=ag$Er[,1],Em=ag$Em[,1],
...
output_MB_sd <- data.frame(time=ag$time ,S=ag$S[,2],Er=ag$Er[,2],Em=ag$Em[,2],
...
output_MB_int <- data.frame(time=ag_int$time ,S=ag_int$S[,1],Er=ag_int$Er[,1],Em=ag_int$Em[,1],
...
output_MB_sd_int <- data.frame(time=ag_int$time ,S=ag_int$S[,2],Er=ag_int$Er[,2],Em=ag_int$Em[,2],
...
#odesolver output
#output_MB <- as.data.frame(ode(y = make_init(), times = times, func = sveirs , parms = parameters))
output_MB <- output_MB %>% mutate(total_pop =S+Er+Em+Ir+Im+R+V+Erv+Emv+Irv+Imv+Rv+W+Erw+Emw+Irw+Imw+Rw)


#helper fuctions 

fit = readRDS("/Users/elishaare/Desktop/PHAC_forecasts_14DEC2021/COVID-PHAC-forecasts/data-generated/MB-fit.rds")
dat = readRDS("/Users/elishaare/Desktop/PHAC_forecasts_28DEC2021/COVID-PHAC-forecasts/data-generated/MB-dat.rds")


incid = get_total_incidence(output=output_MB,parameters=c(mean(beta_r),parameters)) #set output to Province output 
...
incid_sd_MB = get_total_incidence(output=output_MB_sd,parameters=c(mean(beta_r),parameters)) #set output to Province output 
incid_sd_MB = incid %>% select(time, inc_res, inc_mut, inc_tot)
incid_sd_MB = incid_sd_MB %>% mutate(err=qt(0.975,df=sim_size-1)*inc_tot/sqrt(sim_size))
incid_sd_MB = incid_sd_MB %>% mutate(lower = inc_tot-err, upper = inc_tot+err)

incid_int = get_total_incidence(output=output_MB_int,parameters=c(mean(beta_r),parameters_int)) #set output to Province output 
...
incid_sd_MB_int = get_total_incidence(output=output_MB_sd_int,parameters=c(mean(beta_r),parameters_int)) #set output to Province output 
incid_sd_MB_int = incid_int %>% select(time, inc_res, inc_mut, inc_tot)
incid_sd_MB_int = incid_sd_MB_int %>% mutate(err=qt(0.975,df=sim_size-1)*inc_tot/sqrt(sim_size))
incid_sd_MB_int = incid_sd_MB_int %>% mutate(lower = inc_tot-err, upper = inc_tot+err)



#MB_Omicron_proj <- select(incid, c("date","inc_mut" , "rcases","inc_tot"))
#names(MB_Omicron_proj) <- c("date", "Omicron", "rcases","total")
#MB_Omicron_proj$Omicron <- round(MB_Omicron_proj$Omicron,2)
#MB_Omicron_proj$total <- round(MB_Omicron_proj$total,2)
#MB_Omicron_proj$rcases <- round(MB_Omicron_proj$rcases,2)
#MB_Omicron_proj <- filter(MB_Omicron_proj, date >= ymd("2021-12-10") & date <= ymd("2022-01-10"))
#write.csv(MB_Omicron_proj,'MB_Omicron_proj.csv')


incid_long <- melt(incid,  id.vars = 'time', variMBle.name = 'series')
ggplot(incid_long, aes(time,value)) + geom_line(aes(colour = series))


growth_rate_MB <- get_growth_rate(output_MB)
doubling_time_MB <- get_doubling_time( growth_rate_MB)
get_selection_coef(growth_rate_MB)
check <- get_population_immunity(output_MB,N)
...
#               theta=mean(fit$post$phi)))
saveRDS(incid, file.path("data/MB-incid.rds"))
incid =readRDS(file.path("data/MB-incid.rds"))

saveRDS(incid_sd_MB, file.path("data/MB-incid_er.rds"))
incid_sd_MB =readRDS(file.path("data/MB-incid_er.rds"))


saveRDS(incid_int, file.path("data/MB-incid_int.rds"))
incid_int =readRDS(file.path("data/MB-incid_int.rds"))

saveRDS(incid_sd_MB_int, file.path("data/MB-incid_er_int.rds"))
incid_sd_MB_int =readRDS(file.path("data/MB-incid_er_int.rds"))


#set path to recent PHAC forecasts data
#fit = readRDS("/Users/elishaare/Desktop/PHAC_forecasts_14DEC2021/COVID-PHAC-forecasts/data-generated/MB-fit.rds")
#dat = readRDS("/Users/elishaare/Desktop/PHAC_forecasts_14DEC2021/COVID-PHAC-forecasts/data-generated/MB-dat.rds")
...
gg_MB <- plot_projection(modelproj, dat, date_column = "date") +
  theme(axis.title.x = element_blank()) +
  # geom_line(data=incid, aes(x=date, y=inc_mut, col ="Omicron"), size=1.4) +
  geom_ribbon(data=incid, aes(x=date, ymin = incid_sd_MB$lower, ymax = incid_sd_MB$upper),
              inherit.aes = FALSE, fill = "grey", alpha = 0.5) +
  
  geom_line(data=incid_int, aes(x=date, y=inc_tot, col ="75% reduction in transmission"), 
            size=1, alpha = 0.5) +
  geom_ribbon(data=incid_int, aes(x=date, ymin = incid_sd_MB_int$lower, ymax = incid_sd_MB_int$upper),
...
#saveRDS(g, file.path("figs/Show_freqMB.rds"))
