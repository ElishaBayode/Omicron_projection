...
test_prop <- 1 - 0.5/(1+ exp(-1.25*(1:nrow(dat_omic)-15)))
test_prop1 <- 1 - 0.85/(1+ exp(-1.25*(1:nrow(dat_omic)-100)))
...
######## forecast cases ############### 
SK_forecast <- case_projection(out_state=out_state,
                               forecast_days=30, 
                               lag=0,
...
estim_parameters <- c(parameters_2,mle_est_SK)

SK_forecast_int <- case_projection(out_state=out_state,
                                   forecast_days=30, 
                                   lag=0,
                                   parameters_estim = estim_parameters, 
                                   simu_size = 100000,
...
gg_SK <- ggplot() + geom_line(data=dat_sim,aes(x=date,y=`50%`),color='blue',size=1.2,alpha=0.4) +
  ...
  geom_line(data=SK_forecast,aes(x=date, y=`50%`,color="Current"),size=1.2,alpha=0.5) +
  geom_ribbon(data=SK_forecast,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill='orange',alpha=0.1)+
  geom_line(data=SK_forecast_int,aes(x=date, y=`50%`, color="50% reduction in transmission"),size=1.2,alpha=0.5) +
  geom_ribbon(data=SK_forecast_int,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill='green',alpha=0.1)+
