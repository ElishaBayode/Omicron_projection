...
#including Omicron wave only 
dat <- filter(dat, date <= ymd("2022-01-17"))

#dat_omic <- filter(dat, date <= ymd("2022-01-01"))
...
test_prop_SK <- filter(mytest_SK, date >= intro_date)$test_prop
test_prop_SK =  test_prop_AB
...
fit_SK <- optim(fn=f_loglik,par=guess, lower=c(log(0.3), 0.1, log(1.4), log(0.1)), 
                upper = c(log(0.0), 0.6, log(2.1), log(0.12)), method = "L-BFGS-B")
...
estim_parameters <- c(parameters_1,mle_est_SK)


#an attempt to model changes in ascertainment probSKility 
...
dat_omic_end <- filter(dat, date > restart_date & date <= ymd("2022-01-17") )
...
#this catches estimated parameter values from MLE 
mle_est_SK2 <- c(beta_m=exp(fit_SK2$par[1]),p=expit(fit_SK$par[2]), beta_r=exp(fit_SK2$par[3]),theta=exp(fit_SK$par[4]))

signif(mle_est_SK2,3)
...
coef(pomp_obj) <- c(c(S_0=last(out_state["S",,]),Er_0=last(out_state["Er",,]),Em_0=last(out_state["Em",,]),
                      ...
                      c(parameters,mle_est_SK2)))


#


estim_parameters <- c(parameters_1,mle_est_SK2)
...
model.pred <- c(model.pred_fake,model.pred_2)*test_prop #test_prop_SK
...
#                                        parms=c(parameters_1,mle_est_AB)))       
# 
...
######## forecast cases ############### 
SK_forecast <- case_projection(out_state=out_state_2,
                               forecast_days=30, 
                               lag=0,
                               test_prop = test_prop_SK,
...
estim_parameters <- c(parameters_2,mle_est_SK2)

SK_forecast_int <- case_projection_rel(out_state=out_state,
                                       forecast_days=30, 
                                       lag=0,
                                       parameters_estim = estim_parameters, 
                                       simu_size = 100000,
...
saveRDS(dat_sim, file.path("data/SK_dat_sim.rds"))
dat_sim =readRDS(file.path("data/SK_dat_sim.rds"))

saveRDS(dat_sim_rel, file.path("data/SK_dat_sim_rel.rds"))
dat_sim_rel =readRDS(file.path("data/SK_dat_sim_rel.rds"))

saveRDS(SK_forecast, file.path("data/SK_forecast.rds"))
SK_forecast =readRDS(file.path("data/SK_forecast.rds"))

saveRDS(SK_forecast_int, file.path("data/SK_forecast_int.rds"))
SK_forecast_int =readRDS(file.path("data/SK_forecast_int.rds"))


cols <- c("Current, with testing constraints" = "darkgreen", "Without testing constraints"="orange")

gg_SK <- ggplot() + geom_line(data=dat_sim, aes(x=date ,y =`50%`),color='blue',size=1.2,alpha=0.4) +
  ...
  geom_line(data=SK_forecast,aes(x=date, y=`50%`,color="Current, with testing constraints"),size=1.2,alpha=0.5) +
  geom_ribbon(data=SK_forecast,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill='darkgreen',alpha=0.1)+
  geom_line(data=SK_forecast_int,aes(x=date, y=`50%`, color="Without testing constraints"),size=1.2,alpha=0.5) +
  geom_ribbon(data=SK_forecast_int,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill='orange',alpha=0.1)+
