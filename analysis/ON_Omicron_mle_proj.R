...
#an attempt to model changes in ascertainment probONility 
...
######## forecast cases ############### 
ON_forecast <- case_projection(out_state=out_state,
                               forecast_days=30, 
                               lag=0,
...
estim_parameters <- c(parameters_2,mle_est_ON)

ON_forecast_int <- case_projection(out_state=out_state,
                                   forecast_days=30, 
                                   lag=0,
                                   parameters_estim = estim_parameters, 
                                   simu_size = 100000,
...
gg_ON <- ggplot() + geom_line(data=dat_sim,aes(x=date,y=`50%`),color='blue',size=1.2,alpha=0.4) +
  ...
  geom_line(data=ON_forecast,aes(x=date, y=`50%`,color="Current"),size=1.2,alpha=0.5) +
  geom_ribbon(data=ON_forecast,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill='orange',alpha=0.1)+
  geom_line(data=ON_forecast_int,aes(x=date, y=`50%`, color="50% reduction in transmission"),size=1.2,alpha=0.5) +
  geom_ribbon(data=ON_forecast_int,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill='green',alpha=0.1)+
  #geom_line(aes(y=typical),color='blue') +
  labs(y="Reported cases",x="Date") + ylim(c(0,400000)) + 
