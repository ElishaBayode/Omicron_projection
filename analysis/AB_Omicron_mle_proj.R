...
guess <- c(log(1.3), logit(0.99), log(2.2), log(0.01))

#the parameters are constrained  accordingly (lower and upper)

fit_AB <- optim(fn=f_loglik,par=guess, lower=c(log(0.6), 0.5, log(2), log(0.1)), 
                upper = c(log(1.6), 0.9, log(3), log(0)), method = "L-BFGS-B")
...
test_prop <- 1 - 0.55/(1+ exp(-1.25*(1:nrow(dat_omic)-19)))
...
estim_parameters <- c(parameters_2,mle_est_AB)

AB_forecast_int <- case_projection(out_state=out_state,
                                   forecast_days=30, 
                                   lag=0,
                                   parameters_estim = estim_parameters, 
                                   simu_size = 100000,
...
gg_AB <- ggplot() + geom_line(data=dat_sim,aes(x=date,y=`50%`),color='blue',size=1.2,alpha=0.4) +
  ...
  geom_line(data=AB_forecast,aes(x=date, y=`50%`,color="Current"),size=1.2,alpha=0.5) +
  geom_ribbon(data=AB_forecast,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill='orange',alpha=0.1)+
  geom_line(data=AB_forecast_int,aes(x=date, y=`50%`, color="50% reduction in transmission"),size=1.2,alpha=0.5) +
  geom_ribbon(data=AB_forecast_int,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill='green',alpha=0.1)+
