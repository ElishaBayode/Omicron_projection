...
mle_est_AB <- c(beta_r=exp(fit_AB$par[1]),p=expit(fit_AB$par[2]), beta_m=exp(fit_AB$par[3]),theta=exp(fit_AB$par[4]))
...
estim_parameters <- c(parameters_1,mle_est_AB)
...
guess_part2 <- c(log(0.7),logit(0.6), log(1.2), log(0.01))

#the parameters are constrained  accordingly (lower and upper)

fit_AB2 <- optim(fn=f_loglik_2,par=guess_part2, lower=c(log(0.4), 0.1, log(0.9), log(0.4)), 
                 upper = c(log(0.8), 0.3, log(1.5), log(0)), method = "L-BFGS-B")



#this catches estimated parameter values from MLE 
mle_est_AB2 <- c(beta_r=exp(fit_AB2$par[1]),p=expit(fit_AB$par[2]), 
                 beta_m=exp(fit_AB2$par[3]),theta=exp(fit_AB$par[4]))

signif(mle_est_AB2,3)
...
coef(pomp_obj) <- c(c(S_0=last(out_state["S",,]),Er_0=last(out_state["Er",,]),Em_0=last(out_state["Em",,]),
                      ...
                      c(parameters,mle_est_AB2)))


#


estim_parameters <- c(parameters_1,mle_est_AB2)
...
######## forecast cases ############### 
AB_forecast <- case_projection(out_state=out_state,
                               forecast_days=30, 
                               lag=0,
...
estim_parameters <- c(parameters_2,mle_est_AB2)

AB_forecast_int <- case_projection_rel(out_state=out_state_2,
                                       forecast_days=30, 
                                       lag=0,
                                       test_prop=test_prop,
                                       parameters_estim = estim_parameters, 
...
saveRDS(dat_sim, file.path("data/AB_dat_sim.rds"))
dat_sim =readRDS(file.path("data/AB_dat_sim.rds"))

saveRDS(dat_sim_rel, file.path("data/AB_dat_sim_rel.rds"))
dat_sim_rel =readRDS(file.path("data/AB_dat_sim_rel.rds"))

saveRDS(AB_forecast, file.path("data/AB_forecast.rds"))
AB_forecast =readRDS(file.path("data/AB_forecast.rds"))

saveRDS(AB_forecast_int, file.path("data/AB_forecast_int.rds"))
AB_forecast_int =readRDS(file.path("data/AB_forecast_int.rds"))


cols <- c("Current, with testing constraints" = "darkgreen", "Without testing constraints"="orange")

gg_AB <- ggplot() + geom_line(data=dat_sim,aes(x=date,y=`50%`),color='blue',size=1.2,alpha=0.4) +
  ...
  geom_line(data=AB_forecast,aes(x=date, y=`50%`,color="Current, with testing constraints"),size=1.2,alpha=0.5) +
  geom_ribbon(data=AB_forecast,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill='darkgreen',alpha=0.1)+
  geom_line(data=AB_forecast_int,aes(x=date, y=`50%`, color="Without testing constraints"),size=1.2,alpha=0.5) +
  geom_ribbon(data=AB_forecast_int,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill='orange',alpha=0.1)+
