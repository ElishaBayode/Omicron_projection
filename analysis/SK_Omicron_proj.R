...
make_init = function( N=1.174e6, vaxlevel = 0.72,
                      port_wane = 0.1, 
                      past_infection = 0.1, incres = 60, incmut = 4, 
...
parameters <-         c(sigma=1/3, # incubation period (3 days) (to fixed)
                       ...
                       # beta_r=0.5, #transmission rate (to estimate) (0.35)
                        beta_m=0.87*2.2, #transmission rate (to estimate)(*1.9)
                        ...
                        eff_t = as.numeric(eff_date - intro_date) # time 
...
output_SK <- data.frame(time=ag$time ,S=ag$S[,1],Er=ag$Er[,1],Em=ag$Em[,1],
...
output_SK_sd <- data.frame(time=ag$time ,S=ag$S[,2],Er=ag$Er[,2],Em=ag$Em[,2],
...
output_SK_int <- data.frame(time=ag_int$time ,S=ag_int$S[,1],Er=ag_int$Er[,1],Em=ag_int$Em[,1],
...
output_SK_sd_int <- data.frame(time=ag_int$time ,S=ag_int$S[,2],Er=ag_int$Er[,2],Em=ag_int$Em[,2],
...
dat = readRDS("/Users/elishaare/Desktop/PHAC_forecasts_28DEC2021/COVID-PHAC-forecasts/data-generated/SK-dat.rds")


incid = get_total_incidence(output=output_SK,parameters=c(mean(beta_r),parameters)) #set output to Province output 
...
incid_sd_SK = get_total_incidence(output=output_SK_sd,parameters=c(mean(beta_r),parameters)) #set output to Province output 
incid_sd_SK = incid %>% select(time, inc_res, inc_mut, inc_tot)
incid_sd_SK = incid_sd_SK %>% mutate(err=qt(0.975,df=sim_size-1)*inc_tot/sqrt(sim_size))
incid_sd_SK = incid_sd_SK %>% mutate(lower = inc_tot-err, upper = inc_tot+err)

incid_int = get_total_incidence(output=output_SK_int,parameters=c(mean(beta_r),parameters_int)) #set output to Province output 
...
incid_sd_SK_int = get_total_incidence(output=output_SK_sd_int,parameters=c(mean(beta_r),parameters_int)) #set output to Province output 
incid_sd_SK_int = incid_int %>% select(time, inc_res, inc_mut, inc_tot)
incid_sd_SK_int = incid_sd_SK_int %>% mutate(err=qt(0.975,df=sim_size-1)*inc_tot/sqrt(sim_size))
incid_sd_SK_int = incid_sd_SK_int %>% mutate(lower = inc_tot-err, upper = inc_tot+err)



#SK_OmicrSK_proj <- select(incid, c("date","inc_mut" , "rcases","inc_tot"))
#names(SK_OmicrSK_proj) <- c("date", "OmicrSK", "rcases","total")
#SK_OmicrSK_proj$OmicrSK <- round(SK_OmicrSK_proj$OmicrSK,2)
#SK_OmicrSK_proj$total <- round(SK_OmicrSK_proj$total,2)
#SK_OmicrSK_proj$rcases <- round(SK_OmicrSK_proj$rcases,2)
#SK_OmicrSK_proj <- filter(SK_OmicrSK_proj, date >= ymd("2021-12-10") & date <= ymd("2022-01-10"))
#write.csv(SK_Omicron_proj,'ON_Omicron_proj.csv')
...
get_selection_coef(growth_rate_SK)
check <- get_population_immunity(output_SK,N)
...
saveRDS(incid_sd_SK, file.path("data/SK-incid_er.rds"))
incid_sd_SK =readRDS(file.path("data/SK-incid_er.rds"))


saveRDS(incid_int, file.path("data/SK-incid_int.rds"))
incid_int =readRDS(file.path("data/SK-incid_int.rds"))

saveRDS(incid_sd_SK_int, file.path("data/SK-incid_er_int.rds"))
incid_sd_SK_int =readRDS(file.path("data/SK-incid_er_int.rds"))
...
gg_SK <- plot_projection(modelproj, dat, date_column = "date") +
  theme(axis.title.x = element_blank()) +
  # geom_line(data=incid, aes(x=date, y=inc_mut, col ="Omicron"), size=1.4) +
  geom_ribbon(data=incid, aes(x=date, ymin = incid_sd_SK$lower, ymax = incid_sd_SK$upper),
              inherit.aes = FALSE, fill = "grey", alpha = 0.5) +
  
  geom_line(data=incid_int, aes(x=date, y=inc_tot, col ="75% reduction in transmission"), 
            size=1, alpha = 0.5) +
  geom_ribbon(data=incid_int, aes(x=date, ymin = incid_sd_SK_int$lower, ymax = incid_sd_SK_int$upper),
