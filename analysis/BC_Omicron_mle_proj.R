...
guess <- c(log(1.3), logit(0.8), log(2.5), log(0.01))
...
######## forecast cases ############### 
BC_forecast <- case_projection(out_state=out_state,
                         forecast_days=30, 
                         lag=0,
...
estim_parameters <- c(parameters_2,mle_est_BC)

BC_forecast_int <- case_projection(out_state=out_state,
                         forecast_days=30, 
                         lag=0,
                         parameters_estim = estim_parameters, 
                         simu_size = 100000,
...
gg_BC <- ggplot() + geom_line(data=dat_sim,aes(x=date,y=`50%`),color='blue',size=1.2,alpha=0.4) +
  ...
  geom_line(data=BC_forecast,aes(x=date, y=`50%`,color="Current"),size=1.2,alpha=0.5) +
  geom_ribbon(data=BC_forecast,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill='orange',alpha=0.1)+
