...
guess <- c(log(0.81), logit(0.8), log(1.9), log(0.01))

#the parameters are constrained  accordingly (lower and upper)

fit_BC <- optim(fn=f_loglik,par=guess, lower=c(log(0.2), 0.0001, log(1.7), log(0.1)), 
                upper = c(log(0.7), 0.001, log(2.5), log(0)), method = "L-BFGS-B")
...
estim_parameters <- c(parameters_1,mle_est_BC)
...
guess_part2 <- c(log(0.2), logit(0.8), log(0.8),log(0.01))

#the parameters are constrained  accordingly (lower and upper)

fit_BC2 <- optim(fn=f_loglik_2,par=guess_part2, lower=c(log(0.5), 0.0001, log(0.8),  log(0.1)), 
                 upper = c(log(1.2), 0.001, log(5), log(0)), method = "L-BFGS-B")




#this catches estimated parameter values from MLE 
mle_est_BC2 <- c(beta_r=exp(fit_BC2$par[1]),p=expit(fit_BC$par[2]), beta_m=exp(fit_BC2$par[3]),theta=exp(fit_BC$par[4]))

signif(mle_est_BC2,3)
...
coef(pomp_obj) <- c(c(S_0=last(out_state["S",,]),Er_0=last(out_state["Er",,]),Em_0=last(out_state["Em",,]),
  ...
  Irw_0=last(out_state["Irw",,]),Imw_0=last(out_state["Imw",,]), Rw_0=last(out_state["Rw",,]),
                                                                     c(parameters,mle_est_BC2)))


#


estim_parameters <- c(parameters_1,mle_est_BC2)
...
#                                        parms=c(parameters_1,mle_est_BC)))       
# 
...
######## forecast cases ############### 
BC_forecast <- case_projection(out_state=out_state_2,
                         forecast_days=30, 
...
estim_parameters <- c(parameters_2,mle_est_BC2)

BC_forecast_int <- case_projection_rel(out_state=out_state,
                         forecast_days=30, 
                         lag=0,
                         parameters_estim = estim_parameters, 
                         simu_size = 100000,
...
saveRDS(dat_sim, file.path("data/BC_dat_sim.rds"))
dat_sim =readRDS(file.path("data/BC_dat_sim.rds"))

saveRDS(dat_sim_rel, file.path("data/BC_dat_sim_rel.rds"))
dat_sim_rel =readRDS(file.path("data/BC_dat_sim_rel.rds"))

saveRDS(BC_forecast, file.path("data/BC_forecast.rds"))
BC_forecast =readRDS(file.path("data/BC_forecast.rds"))

saveRDS(BC_forecast_int, file.path("data/BC_forecast_int.rds"))
BC_forecast_int =readRDS(file.path("data/BC_forecast_int.rds"))


cols <- c("Current, with testing constraints" = "darkgreen", "Without testing constraints"="orange")

gg_BC <- ggplot() + geom_line(data=dat_sim,aes(x=date,y=`50%`),color='blue',size=1.2,alpha=0.4) +
  ...
  geom_line(data=BC_forecast,aes(x=date, y=`50%`,color="Current, with testing constraints"),size=1.2,alpha=0.5) +
  geom_ribbon(data=BC_forecast,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill='darkgreen',alpha=0.1)+
  geom_line(data=BC_forecast_int,aes(x=date, y=`50%`, color="Without testing constraints"),size=1.2,alpha=0.5) +
  geom_ribbon(data=BC_forecast_int,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill='orange',alpha=0.1)+
  #geom_line(aes(y=typical),color='blue') +
  labs(y="Reported cases",x="Date") + ylim(c(0,25000)) + 
