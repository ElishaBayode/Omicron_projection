...
#https://files.ontario.ca/moh-covid-19-report-en-2021-12-01.pdf
...
tp_approx <- tp_approx_fit(mytest=mytest_ON, dat= dat_omic, forecasts_days=forecasts_days, howlow = 0.13, 
                           slope = 0.14,  midpoint=34, intro_date= intro_date, stop_date=stop_date)
...
incres_in = 938 #866#(made up factor) # resident strain (delta) incidence at start (938, reported cases) 
incmut_in = 30 # new (omicron) inc at stat (presumably very low)
...
ggsave(file="figs/ON_proj.png", gg_ON, width = 8, height = 8)
...
uncert_bound_ON_rel = raply(simu_size,rnbinom(n=length(incidence_ON_rel),
                                              mu=incidence_ON_rel,
                                              size=1/parameters[["theta"]]))
project_dat_ON_rel =  as.data.frame(aaply(uncert_bound_ON_rel 
                                          ,2,quantile,na.rm=TRUE,probs=c(0.025,0.5,0.975))) %>% 
  mutate(date=seq.Date(ymd(intro_date),
                       ymd(intro_date)-1+length(times), 1))
