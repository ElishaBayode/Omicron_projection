...
test_prop_QC <- c(test_prop_QC[1:length(dat_omic$value)], rep(last(test_prop_QC),forecasts_days))
fake_test_prop_QC <- (1 - (1-0.05)/(1 + exp(-0.2*(1:length(test_prop_QC)-32))))

plot(fake_test_prop_QC)
lines(test_prop_QC)

test_prop_QC <- fake_test_prop_QC[1:length(dat_omic$day)]
...
#load initial state    
init_QC <- c(S=init[[1]],Er=init[[2]],Em=init[[3]],Ir=init[[4]],
...
parameters_QC <- c(sigma=1/3, # incubation period (3 days) (to fixed)
                   gamma=1/(4), #recovery rate (fixed)
                   nu =0.007, #vax rate: 0.7% per day (fixed)
                   mu=1/(82*365), # 1/life expectancy (fixed)
                   w1= 1/(3*365),# waning rate from R to S (fixed)
...
parameters <- c(parameters_QC,mle_est_QC)

#use estimated parameters to make projectons 

out_QC <- as.data.frame(deSolve::ode(y=init_QC,time=times,func= sveirs,
                                     parms=parameters))   
...
#with test_prop 
incidence_QC =  parameters[[1]]*(out_QC$Er + out_QC$Erv + out_QC$Erw +
                                   out_QC$Em + out_QC$Emv +
                                   out_QC$Emw)*fake_test_prop_QC 

#without test_prop 
incidence_QC_rel =  parameters[[1]]*(out_QC$Er + out_QC$Erv + out_QC$Erw +
                                       out_QC$Em + out_QC$Emv +
                                       out_QC$Emw)

uncert_bound_QC = raply(simu_size,rnbinom(n=length(incidence_QC),
                                          mu=coef(pomp_obj ,"p")*incidence_QC,
                                          size=1/coef(pomp_obj ,"theta")))

quantiles_proj_QC =  as.data.frame(aaply(uncert_bound_QC 
                                         ,2,quantile,na.rm=TRUE,probs=c(0.025,0.5,0.975)))

project_dat_QC = quantiles_proj_QC %>% mutate(date=seq.Date(ymd(intro_date),
                                                            ymd(intro_date)-1+length(times), 1))



uncert_bound_QC_rel = raply(simu_size,rnbinom(n=length(incidence_QC_rel),
                                              mu=coef(pomp_obj ,"p")*incidence_QC_rel,
                                              size=1/coef(pomp_obj ,"theta")))

quantiles_proj_QC_rel =  as.data.frame(aaply(uncert_bound_QC_rel,2
                                             ,quantile,na.rm=TRUE,probs=c(0.025,0.5,0.975)))

project_dat_QC_rel = quantiles_proj_QC_rel %>% mutate(date=seq.Date(ymd(intro_date),
                                                                    ymd(intro_date)-1+length(times), 1))
