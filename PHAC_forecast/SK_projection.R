...
tp_approx <- tp_approx_fit(mytest=mytest_AB, dat= dat_omic, forecasts_days=forecasts_days, howlow = 0.2, 
                           slope = 0.14,  midpoint=56, intro_date= intro_date, stop_date=stop_date)
...
past_infection_in = 0.1  #increased this from 0.1 to 0.18 # total in R at start time (336K so far, with 50% asc rate)
...
fit_SK <- optim(fn=func_loglik,  par=guess, lower=c(0,0,0), 
                upper = c(Inf,1,Inf), method = "L-BFGS-B", parameters = parameters,
                test_prop=test_prop, dat_omic=dat_omic)
# check the values:
fit_SK
func_loglik(fit_SK$par, test_prop, dat_omic,parameters) 

#this catches estimated parameter values from MLE , and adds them to 'parameters' structure
parameters[names(guess)] <- fit_SK$par
plot.loglik.info(parameters, 1:nrow(dat_omic), test_prop) # cc's sanity check plot 
gg_SK = simple_prev_plot(parameters, numdays = 190) + ylab("Prevalence")  + 
  labs(title="SK"); gg_SK  # cc's simple prevalence plot 
ggsave(file="figs/SK_prev.png", gg_SK, width = 8, height = 8)
...
times <- 1:(nrow(dat_omic) + forecasts_days)
out_SK <- as.data.frame(deSolve::ode(y=init,time=times,func= sveirs,
                                     parms=parameters)) 
#with test_prop 
incidence_SK =  parameters[["p"]]*parameters[["sigma"]]*(out_SK$Er + out_SK$Erv + out_SK$Erw +
                                                           out_SK$Em + out_SK$Emv +
                                                           out_SK$Emw)*test_prop #extendtp(nrow(out_SK), test_prop)

plot(1:nrow(dat_omic), incidence_SK[1:nrow(dat_omic)])
uncert_bound_SK = raply(simu_size,rnbinom(n=length(incidence_SK),
                                          mu=incidence_SK,
                                          size=1/parameters[["theta"]]))
project_dat_SK =  as.data.frame(aaply(uncert_bound_SK 
...
fit_SK_rel <- optim(fn=func_loglik,  par=guess, lower=c(0,0,0), 
                    upper = c(Inf,1,Inf), method = "L-BFGS-B", parameters = parameters,
                    test_prop=rep(1,nrow(dat_omic)), dat_omic=dat_omic) #set test_prop=1
# check the values:

parameters[names(guess)] <- fit_SK_rel$par

times <- 1:(nrow(dat_omic) + forecasts_days)
out_SK_rel <- as.data.frame(deSolve::ode(y=init,time=times,func= sveirs,
                                         parms=parameters)) 

incidence_SK_rel =  parameters[["p"]]*parameters[["sigma"]]*(out_SK$Er + out_SK$Erv + out_SK$Erw +
                                                               out_SK$Em + out_SK$Emv +
                                                               out_SK$Emw)
