...
theta_2 = 1 / ( 1/mu1 - 1/mu2 + 1/parameters[["theta"]])  #
...
#set reopening to early December levels 
parameters_bc <- parameters
parameters_bc["relx_level"] <- parameters_bc[["stngcy"]]/(1-parameters_bc[["stngcy"]])


rlx_t = data.frame(rlx_t=c(which.max(incidence_BC_rel), which.max(incidence_BC_rel)+10,which.max(incidence_BC_rel)+20))


vary_parameter <- function(x, parameters_bc,init1=init){
  parameters_bc["rlx_t"] <- x
  out_var <- as.data.frame(deSolve::ode(y=init1,time=times,func=sveirs,
                                        parms=parameters_bc)) 
  incidence_var =  parameters_bc[["p"]]*parameters_bc[["sigma"]]*(out_var$Er + out_var$Erv + out_var$Erw +
                                                              out_var$Em + out_var$Emv +
                                                              out_var$Emw)
}
incidence_var <- data.frame(apply(rlx_t, 1, vary_parameter, parameters=parameters_bc))



plot(NA,NA, xlim=c(0,250), ylim=c(0,50000))
lines(incidence_BC_rel)
lines(incidence_var$X1,col="blue")
lines(incidence_var$X2,col="red" )
lines(incidence_var$X3,col="green" )
...
uncert_bound_BC_rel_3 = raply(simu_size,rnbinom(n=length(incidence_var$X3),
                                                mu=incidence_var$X3,
                                                size=1/parameters[["theta"]]))

project_dat_BC_rel_3 =  as.data.frame(aaply(uncert_bound_BC_rel_3 
                                            ,2,quantile,na.rm=TRUE,probs=c(0.025,0.5,0.975))) %>% 
  mutate(date=seq.Date(ymd(intro_date),
                       ymd(intro_date)-1+length(times), 1))
...
cols <- c("Current, TC" = "darkgreen", "NTC"="#0072B2",
          "reopen at peak"="orange", "reopen 10 days after peak"="#0072B2", "reopen 20 days after peak"= "#CC79A7")
...
gg_BC <- ggplot() + geom_line(data=project_dat_BC,aes(x=date,y=`50%`, colour = "Current, TC"),size=1.2,alpha=0.4) +
  ...
  geom_line(data=project_dat_BC_rel_1,aes(x=date,y=`50%`, color="reopen at peak"),size=1.2,alpha=0.4) +
  geom_ribbon(data=project_dat_BC_rel_1,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill="purple",alpha=0.1)+
  
  geom_line(data=project_dat_BC_rel_2,aes(x=date,y=`50%`, color="reopen 10 days after peak"),size=1.2,alpha=0.4) +
  geom_ribbon(data=project_dat_BC_rel_2,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill="#D55E00",alpha=0.1)+
  
  geom_line(data=project_dat_BC_rel_3,aes(x=date,y=`50%`, color="reopen 20 days after peak"),size=1.2,alpha=0.4) +
  geom_ribbon(data=project_dat_BC_rel_3,aes(x=date,ymin=`2.5%`,ymax=`97.5%`),fill="#D55E00",alpha=0.1)+
...
ggsave(file="figs/BC_covidgroup.png", gg_BC, width = 16, height = 12)
