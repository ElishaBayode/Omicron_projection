...
tp_approx <- tp_approx_fit(mytest=mytest_AB, dat= dat_omic, forecasts_days=forecasts_days, howlow = 0.2, 
                           slope = 0.12,  midpoint=46, intro_date= intro_date, stop_date=stop_date)
...
past_infection_in = 0.1 #increased this from 0.1 to 0.18 # total in R at start time (68.2K so far, with 50% asc rate)
incres_in = 124*2#866#(made up factor) # resident strain (delta) incidence at start (124, reported cases) 
incmut_in = 1 # new (omicron) inc at stat (presumably very low, first cases was confirmed on Dec 7)
...
eff_date <-   ymd("2022-01-08")  # intervention date # restrictions were updated on 24 December 
...
fit_MB <- optim(fn=func_loglik,  par=guess, lower=c(0,0,0), 
                upper = c(Inf,1,Inf), method = "L-BFGS-B", parameters = parameters,
                test_prop=test_prop, dat_omic=dat_omic)
# check the values:
fit_MB
func_loglik(fit_MB$par, test_prop, dat_omic,parameters) 

#this catches estimated parameter values from MLE , and adds them to 'parameters' structure
parameters[names(guess)] <- fit_MB$par
plot.loglik.info(parameters, 1:nrow(dat_omic), test_prop) # cc's sanity check plot 
gg_MB = simple_prev_plot(parameters, numdays = 190) + ylab("Prevalence")  + 
  labs(title="MB"); gg_MB  # cc's simple prevalence plot 
ggsave(file="figs/MB_prev.png", gg_MB, width = 8, height = 8)
...
times <- 1:(nrow(dat_omic) + forecasts_days)
out_MB <- as.data.frame(deSolve::ode(y=init,time=times,func= sveirs,
                                     parms=parameters)) 
#with test_prop 
incidence_MB =  parameters[["p"]]*parameters[["sigma"]]*(out_MB$Er + out_MB$Erv + out_MB$Erw +
                                                           out_MB$Em + out_MB$Emv +
                                                           out_MB$Emw)*test_prop #extendtp(nrow(out_MB), test_prop)

plot(1:nrow(dat_omic), incidence_MB[1:nrow(dat_omic)])
uncert_bound_MB = raply(simu_size,rnbinom(n=length(incidence_MB),
                                          mu=incidence_MB,
                                          size=1/parameters[["theta"]]))
project_dat_MB =  as.data.frame(aaply(uncert_bound_MB 
...
fit_MB_rel <- optim(fn=func_loglik,  par=guess, lower=c(0,0,0), 
                    upper = c(Inf,1,Inf), method = "L-BFGS-B", parameters = parameters,
                    test_prop=rep(1,nrow(dat_omic)), dat_omic=dat_omic) #set test_prop=1
# check the values:

parameters[names(guess)] <- fit_MB_rel$par

times <- 1:(nrow(dat_omic) + forecasts_days)
out_MB_rel <- as.data.frame(deSolve::ode(y=init,time=times,func= sveirs,
                                         parms=parameters)) 

incidence_MB_rel =  parameters[["p"]]*parameters[["sigma"]]*(out_MB$Er + out_MB$Erv + out_MB$Erw +
                                                               out_MB$Em + out_MB$Emv +
                                                               out_MB$Emw)
...
gg_MB <- ggplot() + geom_line(data=project_dat_MB,aes(x=date,y=`50%`, colour = "Current, with testing constraints"),size=1.2,alpha=0.4) +
  ...
  labs(y="Reported cases",x="Date") + ylim(c(0,8000)) + 
